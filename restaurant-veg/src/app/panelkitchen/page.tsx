      ...
      {ORDERS.map((order) => (
        <div 
          ...
          className={`flex flex-col h-fit rounded-2xl border-2 overflow-hidden bg-white dark:bg-zinc-900 ${
            order.status === 'urgent' ? 'border-red-600' : 'border-zinc-200 dark:border-zinc-800'
          }`}
        >
          {/* Order Header */}
          <div className={`p-4 flex items-center justify-between ${
            order.status === 'urgent' ? 'bg-red-600 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-300'
          }`}>
            <div className="flex items-center gap-2">
              <span className="text-2xl font-black">#{order.id}</span>
              <span className="text-sm font-bold opacity-80 uppercase">Mesa {order.mesa}</span>
            </div>
          ...
          <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-black/40 flex gap-2">
            <button className="flex-1 py-3 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white rounded-xl font-bold transition-colors">
              VER
            </button>

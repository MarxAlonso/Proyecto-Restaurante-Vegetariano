  ...
  return (
    <aside className="w-20 lg:w-64 bg-white dark:bg-zinc-950 border-r border-zinc-200 dark:border-zinc-800 flex flex-col transition-all">
      <div className="p-6 flex items-center gap-3">
        ...
        <span className="text-xl font-bold hidden lg:inline text-zinc-900 dark:text-white">COCINA</span>
      </div>
      ...
      <nav className="flex-1 px-4 py-8 space-y-4">
        {MENU_ITEMS.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="flex items-center gap-4 p-3 hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-xl transition-colors group"
          >
            <item.icon size={24} className="text-zinc-400 dark:text-zinc-500 group-hover:text-secondary transition-colors" />
            <span className="font-semibold text-zinc-600 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-white hidden lg:inline">{item.label}</span>
          </Link>
        ))}
      </nav>
